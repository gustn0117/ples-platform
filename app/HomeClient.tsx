...
export default function HomeClient({ initialArtists, initialBanners }: HomeClientProps) {
  const { user } = useAuth();
  const [hotArtists, setHotArtists] = useState<Artist[]>(initialArtists);
  ...
  useEffect(() => {
    initStore();
    // Refresh data from client-side store for latest
    ...
    fetch('/api/store')
      ...
      .then((data) => {
        ...
        if (active.length > 0) {
          setBanners(active);
          localStorage.setItem('ples_server_banners', JSON.stringify(active));
        }
      })
      .catch(() => {});
  }, []);
