        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-16 pb-10">
          ...
          <div className="mt-8 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              {limitReached && (
                <span className="text-xs text-gray-500 bg-gray-50 px-3.5 py-1.5 rounded-full border border-gray-100 font-medium">내일 다시 방문해주세요</span>
              )}
            </div>
            ...
            <div className="flex justify-between mt-3">
              {Array.from({ length: maxDaily }).map((_, i) => (
                <div key={i} className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold transition-all duration-300 ${
                  i < dailyCount ? 'bg-gray-900 text-white shadow-sm' : 'bg-gray-100 text-gray-400'
                }`}>
                  {i < dailyCount ? <IconCheck className="w-3.5 h-3.5" /> : i + 1}
                </div>
              ))}
            </div>
          </div>
